...
# === Decodificación del modelo secuencial ===
...
# === Procesamiento desde imagen en bytes (para FastAPI) ===
def procesar_imagen_desde_bytes(image_bytes):
    np_array = np.frombuffer(image_bytes, np.uint8)
    img_bgr = cv2.imdecode(np_array, cv2.IMREAD_COLOR)
    return procesar_frame(img_bgr)

# === Procesar un solo frame (vectorización de mano) ===
...
# === Iniciar hilo solo al ejecutar directamente este archivo ===
